import { type TokenCircuitKeys, type TokenProviders } from "../../api/src/index.js";
import { type TokenPrivateState } from "../../contract/src/index.js";
...
let cachedProviders: Promise<TokenProviders> | undefined;

export const getProviders = (): Promise<TokenProviders> => {
  return cachedProviders ?? (cachedProviders = initializeProviders());
};

const initializeProviders = async (): Promise<TokenProviders> => {
  ...
  const keyMaterialProvider = new FetchZkConfigProvider<TokenCircuitKeys>(zkConfigPath, fetch.bind(window));
  const config = await connectedAPI.getConfiguration();
  console.log("[providers] Wallet config:", JSON.stringify(config, null, 2));
  const privateStateProvider = inMemoryPrivateStateProvider<string, TokenPrivateState>();
