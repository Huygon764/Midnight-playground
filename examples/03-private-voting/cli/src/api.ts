...
import { PrivateVoting, type VotingPrivateState, witnesses } from "@example/private-voting-contract";
...
import { type VotingCircuits, type VotingProviders, type DeployedVotingContract } from "./common-types.js";
...
export const getVotingState = async (
  ...
  return { yesVotes: l.yesVotes, noVotes: l.noVotes, votingOpen: l.votingOpen };
};
...
export const registerVoter = async (
  ...
  const result = await contract.callTx.registerVoter(commitment);
...
export const castVote = async (
  ...
  const result = await contract.callTx.castVote(vote);
  return result.public;
};

export const closeVoting = async (contract: DeployedVotingContract): Promise<FinalizedTxData> => {
  const result = await contract.callTx.closeVoting();
  return result.public;
};
...
export const configureProviders = async (ctx: WalletContext, config: Config): Promise<VotingProviders> => {
  const walletAndMidnightProvider = await createWalletAndMidnightProvider(ctx);
  const zkConfigProvider = new NodeZkConfigProvider<VotingCircuits>(contractConfig.zkConfigPath);
  return {
    privateStateProvider: levelPrivateStateProvider<typeof import("./common-types.js").VotingPrivateStateId>({
