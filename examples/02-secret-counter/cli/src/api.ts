...
import { SecretCounter, type SecretCounterPrivateState, witnesses } from "@example/secret-counter-contract";
...
import {
  type SecretCounterCircuits,
  type SecretCounterProviders,
  type DeployedSecretCounterContract,
...
const compiledContract = CompiledContract.make("secret-counter", SecretCounter.Contract).pipe(
...
): Promise<{ round: bigint; owner: Uint8Array } | null> => {
  assertIsContractAddress(contractAddress);
  const state = await providers.publicDataProvider
    .queryContractState(contractAddress)
    .then((contractState) => {
      if (contractState == null) return null;
      const l = SecretCounter.ledger(contractState.data);
      return { round: l.round, owner: l.owner };
    });
  return state;
};
...
): Promise<DeployedSecretCounterContract> => {
  const contract = await deployContract(providers, {
    compiledContract,
    privateStateId: "secretCounterPrivateState",
...
export const increment = async (contract: DeployedSecretCounterContract): Promise<FinalizedTxData> => {
  const result = await contract.callTx.increment();
  return result.public;
};

export const decrement = async (contract: DeployedSecretCounterContract): Promise<FinalizedTxData> => {
  const result = await contract.callTx.decrement();
  return result.public;
};
...
const registerForDustGeneration = async (
  ...
  if (state.dust.availableCoins.length > 0) {
    console.log(`  + Dust tokens already available (${formatBalance(state.dust.walletBalance(new Date()))} DUST)`);
    return;
  }
  ...
  if (nightUtxos.length === 0) {
    await withStatus("Waiting for dust tokens", () =>
      Rx.firstValueFrom(
        wallet.state().pipe(Rx.throttleTime(5_000), Rx.filter((s) => s.isSynced), Rx.filter((s) => s.dust.walletBalance(new Date()) > 0n)),
      ),
    );
    return;
  }
  await withStatus(`Registering ${nightUtxos.length} NIGHT UTXO(s) for dust generation`, async () => {
    const recipe = await wallet.registerNightUtxosForDustGeneration(
      nightUtxos, unshieldedKeystore.getPublicKey(), (payload) => unshieldedKeystore.signData(payload),
    );
...
export const configureProviders = async (ctx: WalletContext, config: Config): Promise<SecretCounterProviders> => {
  const walletAndMidnightProvider = await createWalletAndMidnightProvider(ctx);
  const zkConfigProvider = new NodeZkConfigProvider<SecretCounterCircuits>(contractConfig.zkConfigPath);
  return {
    privateStateProvider: levelPrivateStateProvider<typeof import("./common-types.js").SecretCounterPrivateStateId>({
